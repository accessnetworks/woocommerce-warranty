			...
			for ( $x = 0; $x < $count; $x++ ) {
				...
				if ( $types[ $x ] == 'no_warranty' ) {
					...
					for ( $i = 0; $i < count( $amounts ); $i++ ) {
						if ( ! isset( $amounts[ $i ] ) || ! isset( $values[ $i ] ) || ! isset( $durations[ $i ] ) ) {
							continue;
						}
		...
		public function create_warranty() {
			...
			if ( is_wp_error( $request_id ) ) {
				...
				if ( $order ) {
					...
					$message = sprintf(
						__( '<a href="admin.php?page=warranties&s=%1$s">RMA (%2$s)</a> has been created for %3$s', 'wc_warranty' ),
						$rma,
						$rma,
							...
							<div class="warranty-request" id="warranty_request_<?php echo $request_id; ?>">
								...
								<fieldset class="inline-edit-col-right">
									<div class="inline-edit-col">
										...
										<div class="field">
											<span class="label"><?php printf( __( '%1$s &times; %2$d', 'wc_warranty' ), get_the_title( $request_product['product_id'] ), $request_product['quantity'] ); ?></span>
										</div>
										<?php endforeach; ?>
