		...
		if ( ! empty( $request['return_tracking_code'] ) ) {
			...
			<div class="store-tracking-code-container">
				...
				<ul class="warranty-data">
					...
					<li><?php printf( __( 'Tracking Code: %1$s &mdash; %2$s', 'wc_warranty' ), $request['return_tracking_code'], '<a href="' . $link . '" target="_blank">' . __( 'Track Return Shipment', 'wc_warranty' ) . '</a>' ); ?></li>
				</ul>
			</div>
				<?php
			} else {
...
function warranty_get_warranty_string( $product_id = 0, $warranty = null ) {
	...
	if ( empty( $warranty ) || $warranty['type'] == 'no_warranty' ) {
		...
		if ( $warranty['length'] == 'lifetime' ) {
			...
			$string = sprintf( __( 'Warranty Included (%1$d %2$s)', 'wc_warranty' ), $warranty['value'], $duration );
		}
	} else {
		...
		foreach ( (array) $warranty['addons'] as $addon ) {
			...
			$string  .= sprintf( __( '%1$d %2$s for %3$s; ', 'wc_warranty' ), $addon['value'], $duration, $amount );
		}
	}
...
function warranty_get_date( $order_date, $warranty_duration, $warranty_unit ) {
	...
	if ( $warranty_unit == 'days' ) {
		...
		if ( ( $warranty_month == 2 ) && ( $warranty_day > 28 ) ) {
			$warranty_day = 29;
		}
