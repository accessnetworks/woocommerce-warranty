<p class="order-info">
	...
	printf(
		__( 'Order <mark class="order-number">%1$s</mark> made on <mark class="order-date">%2$s</mark>.', 'wc_warranty' ),
		...
		foreach ( $results as $result ) {
			...
			if ( ! empty( $result['refunded'] ) && $result['refunded'] == 'yes' ) {
				...
				if ( $refund_amount && $refund_date ) {
					...
					$warranty_details[] = sprintf( __( 'Item has been refunded the amount of %1$s on %2$s', 'wc_warranty' ), wc_price( $refund_amount ), $pretty_date );
				} else {
